...
3. **NLContextual embeddings A/B + recall@3 evaluation harness.** The hierarchical retriever ships with a 0.40 cosine grounding gate that was tuned by hand. A golden QA set under `golden/` plus a Python harness that scores recall@3 across gte-small vs `NLContextualEmbedding` would let us pick the cheaper Apple-native option if recall holds, and would auto-detect drift when chunk schemas evolve.

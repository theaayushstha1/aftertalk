...
The same audio session that played the answer is configured to keep the microphone armed throughout, so the user can interrupt if they want to ask a follow up. In the shipped build the auto interrupt is disabled and the user has to hold the button again, because the energy based voice activity gate that drives the auto interrupt would misfire on the tail of Kokoro audio bleeding past the echo cancellation. This is documented in the decisions section.

## The audio session story

The single most fragile part of the application is the audio session. iOS exposes a global AVAudioSession object that every audio aware framework on the device shares. Misconfiguring it produces silent failures — the microphone goes hot but the speaker stays silent, or echo cancellation fails open and Kokoro's own voice gets fed back into Moonshine. The application uses two distinct configurations. While recording a meeting it uses the playAndRecord category in measurement mode, with echo cancellation off, because Whisper class speech recognition models were trained on relatively unprocessed audio and the voice processing IO unit in voiceChat mode measurably degrades word error rate on free form speech. While answering a question it uses playAndRecord in voiceChat mode, with echo cancellation on, because Kokoro plays back through the same active session and the cancellation is required to keep the answer voice from leaking into the recognizer. Switching between the two configurations is the single most common source of audio session error code minus fifty in the device logs, and the orchestrator falls through gracefully rather than failing the request when this happens.
...
Every Logger call across the application uses the os Logger framework with the privacy parameter explicitly set. Counts, durations, error types, sample rates, and file basenames are public. Anything that could carry user content — the question text, the answer text, the transcript text, speaker names, even short previews — is either omitted entirely or marked as private. A recent code review pass tightened the previously interpolated previews ("term=…", "q=…", forty character chains) to length only fields ("termLen=…", "qLen=…", "chars=…"). No raw transcript or question or answer leaks anywhere into the device console.
...
The unit test suite runs thirty nine tests across five suites in roughly one hundred ten milliseconds on the iPhone 17 Pro Max simulator. Coverage focuses on the modules where a regression would be invisible at runtime — the energy voice activity gate's frame decision logic, the sentence boundary detector's cursor invariants, the meeting title sanitizer's filler stripping, the BM25 index's tokenization, the Reciprocal Rank Fusion implementation including the BM25 only hit survival case, and the diarization service's spurious cluster collapse including the ghost cycle bug regression. There are seven Reciprocal Rank Fusion tests, six energy gate tests, seven sentence boundary detector tests, seven title sanitizer tests, three diarization tests, and a handful of helper tests. Every commit runs the suite before push.

Performance is sampled by an in process SessionPerfSampler that writes a CSV at every recording. The CSV captures timestamps, memory, central processing unit, thermal state, and battery delta over the session. Recent on device testing shows the application sitting around one and a half gigabytes of resident memory after a five minute recording and a Q&A turn, with a peak around two gigabytes at startup that settles within a few seconds. The iPhone 17 Pro Max stays in fair thermal state for a forty minute recording plus question and answer session. The iPhone Air requires the deferred Kokoro warm and the five second graph pin to stay under jetsam.
